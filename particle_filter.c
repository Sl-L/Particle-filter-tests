...
void update_weights(Particle *particles[], int particle_count,
    ...
    float *temp_weights = (float*)malloc(particle_count * sizeof(float));
...
void resample_particles(Particle *particles[], int particle_count) {
    Particle **new_particles = (Particle**)malloc(particle_count * sizeof(Particle *));
    if (!new_particles) {
        perror("Failed to allocate new particles");
        exit(EXIT_FAILURE);
    }
...
int main() {
    ...
    for (int t = 0; t < SIMULATION_STEPS; t++) {
        ...
        update_weights(particles, NUM_PARTICLES, beacons, NUM_BEACONS,
                     ...
                     RSSI0, RSSI_NOISE_STD, observed_rssi);
        resample_particles(particles, NUM_PARTICLES);
